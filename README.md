...
| **CU03** | Gestion de Proyectos y Espacios de Trabajo | Completado | Administracion integral de proyectos en el Dashboard y `/projects`: creacion de proyectos, metadatos, tags, busqueda, version semantica v1.0.0 y **clonacion profunda** de clases y relaciones. |
| **CU04** | Auditoria de Bitacora Global y Eventos de Seguridad | Completado | Registro inmutable de eventos en `audit_logs` con IP del cliente, timestamp, enriquecimiento de usuario, paginacion (20/pag), filtros avanzados y exportacion CSV/JSON para SUPER_ADMIN (`/admin/audit`). |
| **CU05** | Trazabilidad de Proyecto y Papelera | Completado | Timeline cronologico de mutaciones (`diagram_history`) con diff antes/despues, enriquecimiento de autores, papelera de reciclaje con restauracion reversible y purga fisica definitiva en cascada (hard delete). |
| **CU06** | Tutorial Guiado de Onboarding (< 2 min) | Completado | Asistente interactivo guiado paso a paso con spotlight SVG sin desenfoque, boton burbuja de ayuda rapida `?` en el editor, gatillado automatico para nuevos arquitectos/colaboradores y persistencia en `user_profiles.preferences`. |
| **CU07** | Creacion de Proyectos desde Plantilla Base | Completado | Catalogo interactivo de plantillas de dominio (Academico, Hospitalario, Facturacion y Blanco), scaffolding profundo transaccional de nodos y relaciones OMG UML 2.5, remapeo consistente de IDs y trazabilidad en bitacora e historial. |
