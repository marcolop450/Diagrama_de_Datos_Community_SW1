...
export const SettingsPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 relative select-none font-sans">
      <AuroraBackground opacity={0.65} />

      {/* Top Navigation Bar */}
      <header className="h-16 bg-slate-900/80 backdrop-blur-xl border-b border-slate-800 sticky top-0 z-30 px-4 sm:px-8 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate('/dashboard')}
            className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-xl border bg-slate-800 hover:bg-slate-750 border-slate-700 text-slate-300 hover:text-white transition-all cursor-pointer"
          >
            <ArrowLeft size={14} />
        ...
        {/* Right header: Role and Plan Badges */}
        <div className="flex items-center gap-2">
          <span className="px-2.5 py-1 rounded-lg text-[11px] font-mono font-semibold border bg-blue-500/10 border-blue-500/30 text-blue-400">
            {user?.role || 'ARQUITECTO'}
          </span>
          <span className="px-2.5 py-1 rounded-lg text-[11px] font-mono font-semibold hidden sm:inline border bg-purple-500/10 border-purple-500/30 text-purple-400">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 relative z-10">
        <div className="mb-8">
          ...
          <p className="text-xs sm:text-sm mt-1 text-slate-400">
            Personaliza tu identidad de arquitecto, preferencias del lienzo CASE y seguridad
          </p>
        </div>

        {/* Navigation Tabs */}
        <div className="flex items-center gap-1 border-b border-slate-800 mb-8 overflow-x-auto pb-px">
          <button
        ...
        {/* TAB 1: USER PROFILE */}
        {activeTab === 'profile' && (
          <div className="p-6 sm:p-8 rounded-3xl border border-slate-800 bg-slate-900/90 shadow-2xl shadow-black/30">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      disabled 
                      value={user?.email || ''} 
                      className="w-full rounded-xl pl-10 pr-3 py-2.5 text-xs border bg-slate-950 border-slate-800 text-slate-400 opacity-70 cursor-not-allowed" 
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      disabled
                      value={user?.subscriptionExpiresAt ? `Activo hasta ${new Date(user.subscriptionExpiresAt).toLocaleDateString()}` : 'Plan Community Ilimitado'}
                      className="w-full rounded-xl pl-10 pr-3 py-2.5 text-xs border bg-slate-950 border-slate-800 text-slate-400 opacity-70 cursor-not-allowed"
                    />
                  </div>
                </div>
        ...
        {/* TAB 2: CANVAS & EDITOR PREFERENCES */}
        {activeTab === 'preferences' && (
          <div className="space-y-6">
            <div className="p-6 sm:p-8 rounded-3xl border border-slate-800 bg-slate-900/90 shadow-2xl shadow-black/30">
              <h2 className="text-base font-bold mb-1 flex items-center gap-2 text-white">
                <Grid className="w-4 h-4 text-indigo-400" />
                <span>Configuración del Lienzo CASE</span>
              </h2>
              <p className="text-xs mb-6 text-slate-400">
                Ajusta las herramientas de modelado visual, alineación automática y frecuencia de guardado.
              </p>

              <div className="space-y-3">
                {/* Grid Toggle */}
                <div className="flex items-center justify-between p-4 rounded-2xl border bg-slate-950/50 border-slate-800/90 hover:bg-slate-950/80 transition-colors">
                  <div>
                    <span className="text-xs font-semibold block text-slate-100">
                      Cuadrícula del Lienzo
                    </span>
                    <span className="text-[11px] text-slate-400">
                      Muestra la matriz de puntos en el fondo del editor UML.
                    </span>
                  </div>
                  <button
                    type="button"
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl border bg-slate-950/50 border-slate-800/90 hover:bg-slate-950/80 transition-colors">
                  <div>
                    <span className="text-xs font-semibold block text-slate-100">
                      Alineación Magnética (Snap to Grid)
                    </span>
                    <span className="text-[11px] text-slate-400">
                      Ajusta automáticamente las clases a la cuadrícula al moverlas o crearlas.
                    </span>
                  </div>
                  <button
                    type="button"
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl border bg-slate-950/50 border-slate-800/90">
                  <div>
                    ...
                    <span className="text-[11px] text-slate-400">
                      Frecuencia de persistencia automática en Supabase PostgreSQL.
                    </span>
                  </div>
                  <select 
                    value={autoSave} 
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl border bg-slate-950/50 border-slate-800/90">
                  <div>
                    ...
                    <span className="text-[11px] text-slate-400">
                      Escala de visión inicial al abrir cualquier diagrama de clases.
                    </span>
                  </div>
                  <select 
                    value={defaultZoom} 
              ...
              <div className="flex justify-end pt-6">
                ...
                >
                  <Save size={15} />
                  <span>{loading ? 'Guardando...' : 'Guardar Preferencias del Editor'}</span>
                </button>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'security' && (
          <div className="p-6 sm:p-8 rounded-3xl border border-slate-800 bg-slate-900/90 shadow-2xl shadow-black/30 max-w-xl">
            <h2 className="text-base font-bold mb-1 flex items-center gap-2 text-white">
